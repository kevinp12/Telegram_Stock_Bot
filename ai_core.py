...
def chat_with_user(
    ...
    if context_symbol:
        data_safety_instruction = f"""
=========================================
⚠️ 【個股數據死線（Hallucination Line）】
=========================================
1. 實時數據鐵律：你必須嚴格且唯一基於下方提供的「市場快照與量化指標」進行分析。
2. 斷路器機制：如果市場快照資料為空、為 None、顯示 "DATA_FETCH_FAILED" 或者是無效字典，代表後端 Python 數據管道超時。
3. 嚴禁編造：在管道超時或無數據的狀況下，你【被絕對禁止】利用自身常識盲猜、預測、或虛構任何即時價格、FVG 區間、POC 控制點或任何數字。
...
【一般對話規則】
=========================================
目前沒有後端指定的個股快照；若使用者在一般對話中詢問市場或股票，請先直接回答可判斷的部分，並引導使用者輸入明確股票代號或使用 /ask、/tech 取得量化快照。不要自行宣稱「數據管道斷裂」或「量化引擎降級」。
""".strip()
