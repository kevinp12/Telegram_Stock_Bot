...
### 📌 2026-05-11 AI 回覆分工升級（資料互通、任務分流）

為了讓每種指令「重點不同但數據一致」，目前已完成 **同資料底座 + 任務導向回覆**：

- **`/news`**：消息面主導（催化劑、事件影響、風險點）
- **`/fin`**：財報與估值主導（關鍵數字、獲利品質、估值判斷）
- **`/tech`**：技術面與 SMC 主導（結構、動能、共振訊號）
- **`/ask`**：單一股票深度戰術回答（整合消息+財報+技術）
- **自然對話（含代號）**：初步分析模式（短、準、先給方向）
- **`/whale`**：大戶/內部人主導（SEC Form 4 + 13F）

#### 🔁 資料互通原則

- 各指令會共用市場快照、新聞、財務、持股與風險資訊（如 VIX）。
- 差異在於 **prompt 任務目標與輸出模板**，不是資料來源割裂。
- 若資料不足，回覆會明確標示「資料不足」，避免 AI 腦補。
...
*   **💬 自然語言對話**：直接輸入文字或股票代號  
    > AI 自動偵測股票代號並切換為「初步分析模式」（偏多/中性/偏空 + 3 點重點），或進行一般對話。

### 📰 新聞與情報系統
*   **📰 智能新聞解讀**：`/news` 或 `/news [代號/主題]`  
    > 自動推播（持股 > 監控 > 宏觀），或查詢特定標的/主題新聞。
    > 智能路由：一般新聞走「消息面主導」，財報新聞自動切換為「財報面主導」。
*   **🚀 產業趨勢速報**：`/theme [主題]`  
...
### 📊 財務分析
*   **📊 個股財報**：`/fin [代號]`  
    > 完整財務數據 + AI 財報重點解讀（財報關鍵數字、獲利品質、估值判斷、後續觀察）。
