...
def run_with_loading(message, loading_text: str, task_fn, error_prefix: str = "處理失敗"):
    ...
    try:
        result = task_fn()
        if loading:
            delete_loading_safe(message, loading)
        return result
    except Exception as exc:
...
def major_news_alert_job() -> None:
    seen_news_urls: OrderedDict[str, float] = OrderedDict()
    while True:
        try:
                ...
                for symbol in watchlist:
                    ...
                    for item in news_items:
                        ...
                        if not url or url in seen_news_urls:
                            continue
                        seen_news_urls[url] = time.time()
                        seen_news_urls.move_to_end(url)
                        while len(seen_news_urls) > 2500:
                            seen_news_urls.popitem(last=False)
...
def sniper_alert_job() -> None:
    ...
    while True:
        ...
        try:
            # 清理過期狙擊快照，避免長時間待機記憶體累積
            now_cleanup = time.time()
            stale_symbols = [
                s for s, v in SNIPER_WATCH_ZONES.items()
                if now_cleanup - float(v.get("last_updated", 0) or 0) > 6 * 3600
            ]
            for s in stale_symbols:
                SNIPER_WATCH_ZONES.pop(s, None)

            # 硬上限：若仍過大，移除最舊快照
            if len(SNIPER_WATCH_ZONES) > 200:
                by_oldest = sorted(
                    SNIPER_WATCH_ZONES.items(),
                    key=lambda kv: float((kv[1] or {}).get("last_updated", 0) or 0),
                )
                for sym, _ in by_oldest[: len(SNIPER_WATCH_ZONES) - 200]:
                    SNIPER_WATCH_ZONES.pop(sym, None)
...
def on_chart(m):
    user_id, user_name = register_user(m)
    status_msg = bot.reply_to(m, "🖼️ 正在生成戰術圖表...")
    ...
    try:
        ...
        logging.exception("chart failed: %s", exc)
        reply(m, f"❌ 圖表指令失敗：{exc}")
    finally:
        delete_loading_safe(m, status_msg)
...
def on_help(m):
    result = run_with_loading(m, "📘 正在整理指令手冊...", lambda: command.frame.help_text(), "指令手冊讀取失敗")
    if result is not None:
        send_paged_message(m.chat.id, result)
...
def on_list(m):
    ...
    result = run_with_loading(m, "📋 正在讀取持股明細...", lambda: command.cmd_list(user_id, page=1), "持股明細讀取失敗")
    if result is not None:
...
def on_fin(m):
    ...
    status_msg = bot.reply_to(m, "📊 正在整理財報與圖表...")
    try:
        try:
...
def on_theory(m):
    ...
    result = run_with_loading(m, "📚 正在整理交易百科內容...", lambda: command.cmd_theory(m.text or ""), "交易百科讀取失敗")
    if result is not None:
        if isinstance(result, (list, tuple)):
            send_paged_message(m.chat.id, result)
        else:
...
def on_log(m):
    result = run_with_loading(m, "🔒 正在讀取系統日誌...", lambda: read_hidden_log_lines(40), "讀取系統日誌失敗")
    if result is None:
        return
    log_lines = result
    if not log_lines:
